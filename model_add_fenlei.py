...
class DSANet(LightningModule):
    ...
    def __build_model(self):
        ...
        if(self.hp.exp_mode==0):
            ...
            dim=7
        elif(self.hp.exp_mode==2):
            dim=7
        elif(self.hp.exp_mode==3):
            dim=5
        elif(self.hp.exp_mode==4):
            dim=9
        elif(self.hp.exp_mode==5):
            dim=9
        elif(self.hp.exp_mode==6):
            dim=3
        elif(self.hp.exp_mode==7):
            dim=3
        elif(self.hp.exp_mode==8):
            dim=3
        ...
        self.out_linear13 = nn.Linear(self.d_model, 128)
        self.out_linear14 = nn.Linear(self.d_model, 128)
        self.out_linear15 = nn.Linear(self.d_model, 128)
        self.out_linear16 = nn.Linear(self.d_model, 128)
        self.out_linear31 = nn.Linear(128, 32) 
        self.out_linear32 = nn.Linear(128, 32) 
        self.out_linear33 = nn.Linear(128, 32) 
        self.out_linear34 = nn.Linear(128, 32) 
        self.out_linear35 = nn.Linear(128, 32) 
        self.out_linear36 = nn.Linear(128, 32) 
        self.out_linear41 = nn.Linear(32, 4) 
        self.out_linear42 = nn.Linear(32, 4) 
        self.out_linear43 = nn.Linear(32, 4) 
        self.out_linear44 = nn.Linear(32, 4) 
        self.out_linear45 = nn.Linear(32, 4) 
        self.out_linear46 = nn.Linear(32, 4) 
        self.out_linear51 = nn.Linear(4, 1)
        self.out_linear52 = nn.Linear(4, 1)
        self.out_linear53 = nn.Linear(4, 1)
        self.out_linear54 = nn.Linear(4, 1)
        self.out_linear55 = nn.Linear(4, 1)
        self.out_linear56 = nn.Linear(4, 1)

        self.out_linear21 = nn.Linear(self.n_head, 1)
        self.out_linear22 = nn.Linear(self.n_head, 1)
    ...
    def forward(self, x):
        ...
        attn3_out = self.active_func(self.out_linear5(self.out_linear4(self.out_linear3(self.out_linear1(attn3_out)))))
        attn4_out = self.active_func(self.out_linear5(self.out_linear4(self.out_linear3(self.out_linear1(attn4_out)))))
        attn5_out = self.active_func(self.out_linear5(self.out_linear4(self.out_linear3(self.out_linear1(attn5_out)))))
        x_target = self.active_func(self.out_linear5(self.out_linear4(self.out_linear3(self.out_linear1(x_target)))))
        
        attn1_horizon = self.active_func(self.out_linear2(attn1_horizon.transpose(1,2)))
        attn2_horizon = self.active_func(self.out_linear2(attn2_horizon.transpose(1,2)))
        ...
        if(self.hp.exp_mode==0):
            ...
            out = torch.cat((x_target,attn1_out,attn1_horizon,attn2_out,attn2_horizon),dim=2)
        elif(self.hp.exp_mode==4):
            out = torch.cat((x_target,attn1_out,attn1_horizon,attn2_out,attn2_horizon,attn3_out,attn3_horizon,attn5_out,attn5_horizon),dim=2)
        elif(self.hp.exp_mode==5):
            out = torch.cat((x_target,attn1_out,attn1_horizon,attn2_out,attn2_horizon,attn3_out,attn3_horizon,attn4_out,attn4_horizon),dim=2)
        elif(self.hp.exp_mode==6):
            out = torch.cat((x_target,attn3_out,attn3_horizon),dim=2)
        elif(self.hp.exp_mode==7):
            out = torch.cat((x_target,attn2_out,attn2_horizon),dim=2)
        elif(self.hp.exp_mode==8):
            out = torch.cat((x_target,attn1_out,attn1_horizon),dim=2)
        elif(self.hp.exp_mode==9):
        ...
        if(self.hp.ar_mode==1):
            return (output,fenlei_out)
        else:
            return (out,fenlei_out)
    ...
    def training_step(self, data_batch, batch_idx):
        ...
        y_hat,fenlei = self.forward(x)
        """ y_hat_2 = torch.where(z<1, y_hat,y)
        # calculate loss
        loss_val = self.loss(y, y_hat_2) """
        loss_val1 = self.loss(y,y_hat)
        loss_val2 = self.loss(fenlei,z)
    ...
    def training_epoch_end(self,outputs):
        ...
        print('sssss')
        np.save('./init_data.npy',loss.squeeze(1).cpu().detach().numpy())
    ...
    def validation_epoch_end(self, outputs):
        ...
        loss = torch.pow(y - y_hat, 2)

        np.save('./init_data2.npy',loss.squeeze(1).cpu().numpy())
        num_var = y.size(-1)
        y = y.view(-1, num_var)
    ...
    def test_epoch_end(self, outputs):
        ...
        y_ori = y
        y_hat = torch.cat(([x['y_hat'] for x in outputs]), 0)
        fenlei = torch.cat(([x['fenlei'] for x in outputs]), 0)
        ...
        score = loss.squeeze(1).cpu().numpy()
        score = np.multiply(score,1-missing)

        np.save('./result/score_{}.npy'.format(self.hp.data_name),score)
        np.save('./result/fenlei_{}.npy'.format(self.hp.data_name),fenlei.squeeze(1).cpu().detach().numpy())
        np.save('./result/label_{}.npy'.format(self.hp.data_name),label)

        init_score = np.load('./init_data.npy')
        print('ssss',len(init_score))
        print(type(init_score))
        print('ssss',len(score))
        print(type(score))
        #init_score[np.where(init_score>10)] = 10
        ...
        """ for i in range(100):
            ...
            file_name = './resultpot_postive_only_day_excludeano.txt'
        ...
        dic = {'y':y_ori.squeeze(1).cpu(),'y_hat':y_hat.squeeze(1).cpu(),'loss':score,'label':label}
        df = pd.DataFrame(dic)
        ...
        for i in range(0,grain+3):
            thres = (max_th2-min_th2)/grain*i+min_th2
            predict, actual = self.point_adjust(fenlei, label, thres=thres)
            #print(predict,actual)
            f1, precision, recall, tp, tn, fp, fn = self.calc_p2p(predict, actual)
            if f1 > max_f12:
                max_f12 = f1
                max_f1_th2 = thres
        file_name = './result_bestf1_add_fenlei3.txt'
        with open(file_name,'a') as f:
            f.write("data_name: {}".format(self.hp.data_name))
            #f.write(" max f1 score is %f and threshold is %f\n" %(max_f1, max_f1_th))
            f.write(" max f1 score is %f and threshold is %f train_loss is %f test_loss is %f\n" %(max_f12, max_f1_th2, np.mean(init_score), np.mean(score)))
        predict1,actual = self.point_adjust(fenlei, label, thres=max_f1_th2)
        predict2,actual = self.point_adjust(score, label, thres=max_f1_th)
        predict = np.logical_or(predict1,predict2)
        f1, precision, recall, tp, tn, fp, fn = self.calc_p2p(predict, actual)
        file_name = './result_bestf1_add_fenlei4.txt'
        with open(file_name,'a') as f:
            f.write("data_name: {}".format(self.hp.data_name))
            #f.write(" max f1 score is %f and threshold is %f\n" %(max_f1, max_f1_th))
            f.write(" max f1 score is %f\n" %(f1))
    ...
    def add_model_specific_args(): # pragma: no cover
        ...
        parser.add_argument('--data_name',  default='15', type=str)
        ...
        parser.add_argument('--ar_mode', type=int, default=1)
        return parser
