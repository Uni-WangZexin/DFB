...
class UniDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.sample_num = max(self.len - window - horizon + 1 - 2, 0)
        self.samples, self.labels = self.__getsamples(values,labels,timestamp)
    ...
    def __getsamples(self, data,labels,timestamp):
        ...
        for i in range(self.sample_num):
            ...
            #Y[i,:] =np.mean(data[i+self.window+self.horizon-1-2:i+self.window+self.horizon-1+2]).astype(np.float64)
            ...
            for j in range(self.n_positive):
                ...
                X[i,self.window,j+1] = torch.from_numpy(np.asarray(np.mean(data[end+self.horizon-1-2:end+self.horizon-1+2])))
            ...
            for j in range(self.n_positive):
                ...
                X[i,self.window,j+self.n_positive+1] = torch.from_numpy(np.asarray(np.mean(data[end+self.horizon-1-2:end+self.horizon-1+2])))
            ...
            for j in range(self.n_random):
                ...
                X[i,self.window,j+2*self.n_positive+1] = torch.from_numpy(np.asarray(np.mean(data[end+self.horizon-1-2:end+self.horizon-1+2])))
            ...
            for j in range(self.n_negative):
                ...
                X[i, self.window ,j+2*self.n_positive+self.n_random+1] = torch.from_numpy(np.asarray(np.mean(data[end+self.horizon-1-2:end+self.horizon-1+2])))
        """ if(self.set_type=='train'):
